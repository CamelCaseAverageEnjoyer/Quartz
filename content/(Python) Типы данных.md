...
2. Iterable $-$ контейнеры, содержащие несколько переменных
	6. *list* $-$ массив (изменяемый): `a = [1, 2, 3]`
	7. *str* $-$ строка (неизменяемый): `a = "bcd"` или `a = 'bcd'`
	8. *tuple* $-$ кортеж (неизменяемый): `a = 1, 2, 3` или `a = (1, 2, 3)`
	9. *dict* $-$ словарь (неизменяемый): `a = {'b': 1, 'd': 2}`
	10. *set* $-$ множество (изменяемый): `a = {1, 1, 2, 3}`
	11. *frozenset* $-$ множество (неизменяемый): `a = frozenset([1, 1, 2, 3])`
...
2. Индексация через квадратные скобки `a[0]`
	...
	4. Шаг индексации $h$ записывается через второе двоеточие: `a[N:M:h]`
		4. Пример: `a[: :2]` $\to$ от $0$ по последний элементы c шагом $2$
...
4. Можно проверить наличие элемента в контейнере функцией `in`:
	4. Пример: `1 in [1,2,3]` $\to$ `True`
	5. Пример: `'b' in 'abcd'` $\to$ `True`
