...
3. Иерархия графических объектов:
	...
	2. *Figure* $-$ отдельное графическое окно (дескриптор: N=1,2,3...)
		4. Создаётся командами `plot, surf, contour...`
		5. `figure` $-$ открыть графическое окно
		6. `gcf` $-$ получить дескриптор текущего объекта *figure*
		7. `clf` $-$ очистить графическое окно
		8. `close` $-$ закрыть графическое окно
		9. `refresh` $-$ обновить графическое окно
		10. Управление свойствами: `set(N), get(N)`
	...
	4. *UI* (Uicontrol) $-$ управляющие элементы пользовательского интерфейса
		6. Объект Uimenu является выпадающим меню, который выполняет функции обратного вызова, когда пользователь активизирует пункт меню
