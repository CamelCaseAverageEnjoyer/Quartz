...
2. Вывод: `print(s)` (`s` использует своё преобразование в строку)
	2. При нескольких позиционных аргументах `print(a, b, c)` выведутся объекты `a`, `b`, `c` через пробел
	3. Можно добавить именованный аргумент `end` для изменения конца строки: `print(a, end='...')`. По умолчанию, в конце строки $-$ символ переноса строки `\n`.
